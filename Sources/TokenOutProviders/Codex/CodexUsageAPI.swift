...
public enum CodexUsageAPI {
    ...
    public static func decodeWindows(from data: Data, now: Date = .now) -> [LimitWindow] {
        ...
        for keys in [["primary_window", "primary"], ["secondary_window", "secondary"]] {
            ...
            // Fall back to the kind's nominal length when the API omits the
            // window duration, so pace metrics still work.
            let duration: TimeInterval? = durationSeconds > 0 ? durationSeconds
                : kind == .session ? 5 * 3600
                : label == "Weekly" ? 7 * 86_400
                : nil
