...
public actor CodexSessionScanner {
    ...
    /// API-equivalent pricing, USD per MTok, verified against
    /// developers.openai.com/api/docs/pricing (2026-07-18). Cached input reads
    /// bill at 10% of the input rate (the published cached-input prices are
    /// exactly 0.1x input across the family). Longest-prefix-first.
    ...
    static func cost(of stat: CodexSessionStat) -> Double {
        ...
        // cached_input_tokens is a subset of input_tokens in Codex's
        // total_token_usage; clamp so malformed rollouts can't go negative.
        let uncachedInput = max(0, stat.input - stat.cachedInput)
        return (Double(uncachedInput) * rate.input
                + Double(min(stat.cachedInput, stat.input)) * rate.input * 0.1
