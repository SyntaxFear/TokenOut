...
public enum ClaudeTranscriptParser {
    ...
    public static func total(entries: [TranscriptEntry], in range: ClosedRange<Date>) -> Totals {
        ...
        for entry in entries where range.contains(entry.timestamp) {
            ...
            let cost = Pricing.cost(model: entry.model, usage: entry.usage, at: entry.timestamp)
    ...
    public static func totals(entries: [TranscriptEntry], in range: ClosedRange<Date>,
        ...
        for entry in entries where range.contains(entry.timestamp) {
            ...
            group.costUSD += Pricing.cost(model: entry.model, usage: entry.usage, at: entry.timestamp).usd
            groups[name] = group
        }
...
public actor ClaudeTranscriptScanner {
    ...
    public func recentEntries(withinDays days: Int = 8) -> [TranscriptEntry] {
        ...
        // Cross-file dedup: resuming a session (--continue / --resume) copies
        // earlier assistant turns into the new transcript with the same message
        // ids — counting those again would double-bill resumed sessions.
        var seen: Set<String> = []
        return all.filter { seen.insert($0.messageID).inserted }
    }
}
