    ...
    public func fetchUsage() async throws -> UsageSnapshot {
        ...
        guard let live = ClaudeCredentials.load() else {
            if ClaudeCredentials.keychainAccessBlocked {
                ...
                throw ProviderError.signedOut(help:
                    "macOS is blocking Keychain access. Click “Always Allow” on the prompt (relaunch TokenOut if it's gone).")
            }
        ...
        return UsageSnapshot(
            ...
            daily: DayStat.aggregate(
                points: entries.map {
                    ($0.timestamp, $0.usage.total, Pricing.cost(model: $0.model, usage: $0.usage, at: $0.timestamp).usd,
                     ClaudeTranscriptParser.modelFamily($0.model))
                },
                days: 92)
        )
