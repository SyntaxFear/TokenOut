...
public final class HistoryStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "tokenout.history")

    public static var defaultURL: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appending(path: "TokenOut/history.jsonl")
    }
    ...
    private static func load(from url: URL, cutoff: Date) -> [HistorySample] {
        ...
        let decoder = JSONDecoder.tokenOut
    ...
    private func append(_ sample: HistorySample) {
        guard let data = try? JSONEncoder.tokenOut.encode(sample) else { return }
        let line = data + Data("\n".utf8)
