...
public struct LimitWindow: Codable, Sendable, Equatable {
    ...
    /// Full window length (5 h session, 7 d weekly…). Enables pace math below.
    ...
    /// How much of the window has elapsed, 0–1. Derived from the reset time
    /// and the window length (start = resetsAt - duration).
    ...
    /// Usage pace vs. time pace: positive = deficit (burning quota faster than
    /// the window elapses), negative = reserve (quota to spare at this pace).
    public func paceDelta(now: Date = .now) -> Double? {
