...
public enum Pricing {
    ...
    /// `date` is when the usage HAPPENED — time-limited pricing (Sonnet 5's
    /// introductory rate) must follow the usage date, not the wall clock, or
    /// historical daily costs silently change after the price switch.
    public static func cost(model: String, usage: TokenUsage,
                            at date: Date = .now) -> (usd: Double, isEstimated: Bool) {
        ...
        // standard $3/$15 starts 2026-09-01 00:00 UTC (epoch 1788220800).
