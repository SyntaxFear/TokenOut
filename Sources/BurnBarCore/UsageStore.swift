...
public struct PersistedState: Codable, Sendable, Equatable {
    ...
    public static func load(from url: URL) -> PersistedState {
        guard let data = try? Data(contentsOf: url),
              let decoded = try? JSONDecoder.burnBar.decode(PersistedState.self, from: data)
        else { return PersistedState() }
        return decoded
    }

    public func save(to url: URL) {
        guard let data = try? JSONEncoder.burnBar.encode(self) else { return }
    ...
    public static var defaultURL: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appending(path: "BurnBar/state.json")
    }
}

extension JSONDecoder {
    public static var burnBar: JSONDecoder {
        let d = JSONDecoder()
...
extension JSONEncoder {
    public static var burnBar: JSONEncoder {
        let e = JSONEncoder()
