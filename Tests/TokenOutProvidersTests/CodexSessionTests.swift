...
@Test func codexRolloutParsesLastCumulativeUsage() {
    ...
    // gpt-5.3-codex must hit its own $1.75/$14 rate, not the gpt-5 fallback.
    var codex = stat!
    codex.model = "gpt-5.3-codex"
    let codexCost = CodexSessionScanner.cost(of: codex)
    let codexExpected = (Double(23258 - 4000) * 1.75 + 4000 * 0.175 + 289 * 14.0) / 1_000_000
    #expect(abs(codexCost - codexExpected) < 0.000001)

    // Malformed rollout with cached > input must clamp, not go negative.
    var weird = stat!
    weird.input = 100
    weird.cachedInput = 500
    weird.output = 0
    #expect(CodexSessionScanner.cost(of: weird) >= 0)
}
