...
@Test func groupTotalsSortByCost() {
    ...
    let entries = [
        entry("claude-opus-4-8", "TokenOut", output: 1_000_000, at: now),        // $25
        entry("claude-sonnet-5", "TokenOut", output: 100_000, at: now),          // $1.50
    ...
    #expect(byProject.map(\.name) == ["TokenOut", "FreeWorker"])
}
