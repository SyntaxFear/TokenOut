...
@Test func sonnet5IntroPricingFollowsUsageDate() {
    let usage = TokenUsage(input: 1_000_000, output: 1_000_000)
    // 2026-08-31 23:59 UTC — still introductory $2/$10.
    let intro = Pricing.cost(model: "claude-sonnet-5", usage: usage,
                             at: Date(timeIntervalSince1970: 1_788_220_799))
    #expect(abs(intro.usd - 12.0) < 0.0001)
    // 2026-09-01 00:00 UTC — standard $3/$15.
    let standard = Pricing.cost(model: "claude-sonnet-5", usage: usage,
                                at: Date(timeIntervalSince1970: 1_788_220_800))
    #expect(abs(standard.usd - 18.0) < 0.0001)
}
