# TokenOut Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A complete, locally installed macOS menu bar app that live-tracks Claude Code, Codex, and Antigravity usage limits, token totals, and cost estimates.

**Architecture:** SwiftPM package with three targets — `TokenOutCore` (models, refresh engine, store; pure logic), `TokenOutProviders` (Claude/Codex/Antigravity, each one folder conforming to `UsageProvider`), `TokenOut` (SwiftUI `MenuBarExtra` app). A bundling script assembles and ad-hoc-signs `TokenOut.app`; a `tokenout-probe` dev CLI captures live API fixtures. UI consumes only the normalized `UsageSnapshot` — providers are plug-ins.
...
**Working directory: `~/Desktop/TokenOut` — all paths below are relative to it. Commit after every task.**
...
```
Package.swift
Sources/TokenOutCore/{Models,UsageProvider,RefreshPolicy,UsageStore,Pricing,Format}.swift
Sources/TokenOutProviders/Claude/{ClaudeProvider,ClaudeCredentials,ClaudeUsageAPI,ClaudeTranscripts}.swift
Sources/TokenOutProviders/Codex/{CodexProvider,CodexAuth,CodexUsageAPI}.swift
Sources/TokenOutProviders/Antigravity/AntigravityProvider.swift
Sources/TokenOutProbe/main.swift                     (dev CLI)
Sources/TokenOutApp/{TokenOutApp,AppState,MenuBarLabel,PopoverView,ProviderCard,SettingsView,OnboardingWindow,Notifier}.swift
Tests/TokenOutCoreTests/{FormatTests,PricingTests,WindowTests,StoreTests}.swift
Tests/TokenOutProvidersTests/{ClaudeTranscriptTests,ClaudeUsageDecodingTests,CodexTests}.swift
Tests/TokenOutProvidersTests/Fixtures/*.json|jsonl
Support/Info.plist
...
let package = Package(
    ...
    targets: [
        ...
        .executableTarget(name: "TokenOut", dependencies: ["TokenOutCore", "TokenOutProviders"],
        ...
        .testTarget(name: "TokenOutProvidersTests", dependencies: ["TokenOutProviders"],
...
- [ ] **Step 3:** Create all listed swift files with `// TokenOut` placeholder comment so targets compile. Run `swift build` → succeeds. Commit `chore: scaffold SwiftPM package`.

### Task 2: Core models + formatting (TDD)

**Files:** `Sources/TokenOutCore/Models.swift`, `Format.swift`; `Tests/TokenOutCoreTests/{WindowTests,FormatTests}.swift`
...
**Files:** `Sources/TokenOutCore/Pricing.swift`; `Tests/TokenOutCoreTests/PricingTests.swift`
...
**Files:** `Sources/TokenOutCore/UsageProvider.swift`
...
**Files:** `Sources/TokenOutCore/UsageStore.swift`; `Tests/TokenOutCoreTests/StoreTests.swift`
...
- [ ] **Step 3:** `@MainActor @Observable public final class UsageStore` holding `states: [ProviderID: ProviderState]` (`ProviderState { snapshot: UsageSnapshot?; status: ProviderStatus; lastErrorText: String? }`), `apply(result:for:)`, plus nested Codable `PersistedState` with static load/save (JSON in `~/Library/Application Support/TokenOut/state.json`, injectable URL for tests). Persistence pure functions live outside `@MainActor` for testability.
- [ ] **Steps 2/4/5:** FAIL → PASS → commit.

### Task 6: Refresh policy (TDD) + engine

**Files:** `Sources/TokenOutCore/RefreshPolicy.swift`; `Tests/TokenOutCoreTests/WindowTests.swift` (append)
...
**Files:** `Sources/TokenOutProviders/Claude/ClaudeCredentials.swift`; test in `ClaudeTranscriptTests.swift` file section
...
**Files:** `Sources/TokenOutProviders/Claude/ClaudeTranscripts.swift`; fixture `Fixtures/claude-transcript.jsonl`; `Tests/TokenOutProvidersTests/ClaudeTranscriptTests.swift`
...
**Files:** `Sources/TokenOutProviders/Claude/{ClaudeUsageAPI,ClaudeProvider}.swift`; fixture `Fixtures/claude-usage.json`; `Tests/TokenOutProvidersTests/ClaudeUsageDecodingTests.swift`
...
**Files:** `Sources/TokenOutProviders/Codex/{CodexAuth,CodexUsageAPI,CodexProvider}.swift`; fixtures `Fixtures/codex-auth.json`, `Fixtures/codex-usage.json`; `Tests/TokenOutProvidersTests/CodexTests.swift`
...
**Files:** `Sources/TokenOutProbe/main.swift`

- [ ] **Step 1:** CLI: `tokenout-probe claude|codex|all` → prints HTTP status + raw JSON (tokens redacted) for each provider's usage endpoint using the credential readers. Used in Task 15 to capture real fixtures.
- [ ] **Step 2:** `swift build` → commit `feat: tokenout-probe dev CLI`.

### Task 12: Antigravity spike + provider

**Files:** `docs/providers/antigravity.md`, `Sources/TokenOutProviders/Antigravity/AntigravityProvider.swift`, tests+fixture if parseable format found
...
**Files:** `Sources/TokenOutApp/{TokenOutApp,AppState,MenuBarLabel,PopoverView,ProviderCard}.swift`
...
- [ ] **Step 3: MenuBarLabel** — drawn template `NSImage` (TokenOut mark + 14×5pt rounded gauge with fill = tightest fraction) + `Text(pct)`; amber/red conveyed by fill + `!` badge (menu bar strips color). `AppState.menuBarMetric` = tightest across enabled providers (fallback "–").
...
- [ ] **Step 5:** `swift build && swift run TokenOut` smoke (menu bar appears) → commit `feat(app): menu bar + popover UI`.

### Task 14: Settings, notifications, onboarding, login item

**Files:** `Sources/TokenOutApp/{SettingsView,Notifier,OnboardingWindow}.swift`
...
- [ ] **Step 3: OnboardingWindow** — first-launch `NSWindow` (NSHostingController): what TokenOut reads (local files/Keychain, goes nowhere), detected tools with checkmarks, "the Keychain prompt is expected — click Always Allow", Start button. `UserDefaults` flag.
...
- [ ] **Step 1: Info.plist** — `LSUIElement=true`, id `dev.tokenout.mac`, name/display TokenOut, `CFBundleShortVersionString 0.9.0`, `LSMinimumSystemVersion 14.0`, `CFBundleIconFile AppIcon`, `NSHumanReadableCopyright`.
- [ ] **Step 2: makeicon.swift** — AppKit script drawing the TokenOut token-ring and escaping-arrow mark on a dark rounded-square background; render all iconset sizes; `iconutil -c icns` → `Support/AppIcon.icns`.
- [ ] **Step 3: bundle.sh** — `swift build -c release` → assemble `dist/TokenOut.app/Contents/{MacOS/TokenOut,Info.plist,Resources/AppIcon.icns,PkgInfo}` → `codesign --force -s - dist/TokenOut.app`. **install-local.sh** — quit running instance, `ditto` to `/Applications`, `open`.
- [ ] **Step 4: LIVE VERIFY (user present):** run `tokenout-probe all` — user clicks Keychain **Always Allow**; capture real response JSON → sanitize → replace best-guess fixtures; adjust decoders if shapes differ; `swift test` green.
- [ ] **Step 5:** `bundle.sh && install-local.sh`; verify menu bar shows real percentages; screenshot popover; README (what it is, build, install, phase 2 roadmap). Final commit `feat: TokenOut 0.9.0 local release`.
