...
A macOS menu bar app that shows live usage, rate-limit windows, and token burn for AI coding tools. v1 tracks **Claude Code**, **Codex**, and **Antigravity**. Sold as a one-time license with a 7-day trial. The pitch mirrors the category leader's but with our own angle: *your token burn, as a literal bar in your menu bar.*

- macOS 14+ (Apple Silicon + Intel universal binary)
- Menu-bar-only app (`LSUIElement`), no Dock icon
- Privacy story: all usage data stays on-device; credentials are read locally and only ever sent to each provider's own official API; the only BurnBar server traffic is license validation and update checks
- Price ballpark at launch: $9 solo / $15 all-tools (final call later; not a build blocker)

## 2. Architecture

Native SwiftUI, Swift 6, structured concurrency. SwiftPM package assembled into `BurnBar.app` by a bundling script (fully headless builds; no `.xcodeproj`). Migrate to XcodeGen only if/when widgets or App Store distribution force it.

SwiftPM targets:

| Target | Purpose |
|---|---|
| `BurnBarCore` | Models, `UsageProvider` protocol, refresh engine, stores, license verifier. Pure logic, no UI, fully unit-testable. |
| `BurnBarProviders` | One folder per provider (Claude, Codex, Antigravity). Depends on Core only. |
| `BurnBar` (app) | MenuBarExtra UI, popover, settings, notifications, onboarding. |
...
- **Menu bar:** flame glyph + a tiny horizontal burn bar (the brand, literally) + optional compact percent. Shows the *tightest* window across enabled providers by default; configurable to pin one provider. Color: normal → amber ≥ 80 % → red ≥ 95 %.
...
- **Onboarding (first launch):** what BurnBar reads and why, Keychain prompt explanation, provider auto-detection, trial start.
...
- **Payments:** provider decision deferred (Paddle vs Lemon Squeezy — merchant-of-record preferred for VAT). Day-one sales work via manual key issuance CLI (`burnbar-keygen`).
...
| Name collision (drink app owns burnbar.app; Apple Fitness+ "Burn Bar" feature) | Different category + direct distribution; trademark diligence before launch; rename cheap pre-launch |
