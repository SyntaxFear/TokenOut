...
RW_DMG="$(mktemp -u /tmp/BurnBar-rw.XXXXXX).dmg"
STAGE="$(mktemp -d /tmp/BurnBar-stage.XXXXXX)"
MOUNT=""
BACKGROUND_MASTER="Brand/marketing/dmg-background-master.png"
...
[[ -f "$BACKGROUND_MASTER" ]] || { echo "Missing $BACKGROUND_MASTER."; exit 1; }

mkdir -p "$STAGE/.background"
ditto "$APP" "$STAGE/BurnBar.app"
ln -s /Applications "$STAGE/Applications"

swift scripts/makedmgbackground.swift "$BACKGROUND_MASTER" "$BACKGROUND"

hdiutil create -volname BurnBar -srcfolder "$STAGE" -ov -format UDRW "$RW_DMG" >/dev/null
...
tell application "Finder"
  tell disk "BurnBar"
    ...
    set position of item "BurnBar.app" of container window to {165, 215}
